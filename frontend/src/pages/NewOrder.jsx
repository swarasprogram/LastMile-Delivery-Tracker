      ...
      <div className="flex flex-1 max-w-6xl mx-auto w-full gap-8 p-8">
        ...
        <div className="flex-1 space-y-6">
              ...
              <div key={s} className="flex items-center gap-2">
                <div className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-semibold transition ${
                  ...
                  <span className={`w-4 h-4 rounded-full flex items-center justify-center text-xs ${
                    i < step ? "bg-blue-400 text-white" : ""
                  }`}>
                    {i < step ? "✓" : i + 1}
                  </span>
                  {s}
                </div>
                {i < steps.length - 1 && <div className={`h-px w-6 ${i < step ? "bg-blue-500" : "bg-gray-700"}`} />}
              </div>
            ))}
          </div>
          ...
          {step === 0 && (
            <div className="space-y-5">
              <div>
                ...
                <p className="text-gray-400 text-sm">Enter pickup and drop locations</p>
              </div>

              <div className="bg-gray-900 border border-gray-800 rounded-2xl p-5 space-y-4">
                <div>
                  <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2 block">Pickup</label>
                  <div className="relative">
                    <div className="absolute left-3 top-1/2 -translate-y-1/2 w-2.5 h-2.5 rounded-full bg-emerald-400" />
                    <AddressAutocomplete
                  ...
                  {pickup.lat && <p className="text-xs text-emerald-500 mt-1.5 ml-1">✓ Location confirmed</p>}
                </div>

                <div className="border-l-2 border-dashed border-gray-700 ml-1 h-4" />

                <div>
                  <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2 block">Drop</label>
                  <div className="relative">
                  ...
                  {drop.lat && <p className="text-xs text-emerald-500 mt-1.5 ml-1">✓ Location confirmed</p>}
                </div>
              </div>

              {/* Inline map preview */}
              {(pickup.lat || drop.lat) && (
                <div className="rounded-2xl overflow-hidden border border-gray-800" style={{ height: "240px" }}>
                  <Suspense fallback={<div className="h-full bg-gray-900 flex items-center justify-center text-gray-500 text-sm">Loading map…</div>}>
                    <MapView
          ...
          {step === 1 && (
            <div className="space-y-5">
              ...
              <div className="grid grid-cols-2 gap-3">
                {PACKAGE_PRESETS.map(p => (
                  <button key={p.label} type="button"
                    ...
                    className={`bg-gray-900 border rounded-2xl p-4 text-left transition hover:border-blue-500 ${
              ...
              <div className="bg-gray-900 border border-gray-800 rounded-2xl p-5 space-y-4">
                <p className="text-gray-400 text-sm font-medium">Dimensions & Weight</p>
                <div className="grid grid-cols-3 gap-3">
                  {[
                    ...
                    <div key={f.key}>
                      ...
                      <input type="number"
                        className="w-full bg-gray-800 border border-gray-700 rounded-xl px-3 py-2.5 text-white text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition"
                        value={dims[f.key]}
                    ...
                    {["B2C","B2B"].map(t => (
                      <button key={t} type="button" onClick={() => setOrderType(t)}
                        className={`flex-1 py-2 rounded-xl text-sm font-medium transition ${
                          orderType === t ? "bg-blue-600 text-white" : "bg-gray-800 text-gray-400 hover:text-white"
                        }`}>
                        {t}
                      </button>
                    ))}
                  ...
                  <div className="flex gap-2">
                    {["Prepaid","COD"].map(t => (
                      <button key={t} type="button" onClick={() => setPaymentType(t)}
                        className={`flex-1 py-2 rounded-xl text-sm font-medium transition ${
                          paymentType === t ? "bg-blue-600 text-white" : "bg-gray-800 text-gray-400 hover:text-white"
                        }`}>
                        {t}
                      </button>
                    ))}
          ...
          {step === 2 && (
            <div className="space-y-5">
              ...
              <div className="bg-gray-900 border border-gray-800 rounded-2xl p-5 space-y-4">
                ...
                <div className="border-l-2 border-dashed border-gray-700 ml-1 h-3" />
                <div className="flex items-start gap-3">
              ...
              {estimating ? (
                ...
                <div className="bg-gradient-to-br from-blue-900/40 to-indigo-900/40 border border-blue-700/30 rounded-2xl p-5">
                  <p className="text-blue-400 text-xs font-semibold uppercase tracking-wider mb-4">Price Breakdown</p>
                  ...
                  <div className="border-t border-blue-700/30 mt-4 pt-4 flex justify-between items-center">
                    ...
                    <span className="text-2xl font-bold text-blue-400">₹{estimate.total_charge}</span>
                  </div>
                  {paymentType === "Prepaid" && (
                    <p className="text-blue-400/60 text-xs mt-3 text-center">You'll pay via Razorpay after placing the order</p>
                  )}
                </div>
              ) : null}

              {error && (
                <div className="bg-red-900/20 border border-red-700/30 text-red-400 text-sm rounded-xl p-4">
                  {error}
                </div>
              )}
            </div>
          ...
          <div className="flex gap-3 pt-2">
            {step > 0 && (
              <button
                ...
                className="flex-1 border border-gray-700 text-gray-300 hover:border-gray-500 hover:text-white py-3 rounded-xl font-semibold text-sm transition"
              >
                ← Back
              </button>
            )}
            {step < 2 ? (
              <button
                onClick={() => setStep(s => s + 1)}
                disabled={!canGoNext}
                className="flex-1 bg-blue-600 hover:bg-blue-500 disabled:opacity-40 disabled:cursor-not-allowed text-white py-3 rounded-xl font-semibold text-sm transition"
              >
                Continue →
              </button>
            ) : (
              <button
                onClick={placeOrder}
                disabled={loading}
                className="flex-1 bg-blue-600 hover:bg-blue-500 disabled:opacity-60 text-white py-3 rounded-xl font-bold text-sm transition shadow-lg shadow-blue-900/30"
              >
        ...
        <div className="hidden lg:block w-80 xl:w-96">
          <div className="sticky top-8">
            <div className="bg-gray-900 border border-gray-800 rounded-2xl overflow-hidden" style={{ height: "500px" }}>
              {(pickup.lat || drop.lat) ? (
